  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-100 dark:bg-gray-900">
      ...
      <div className="container mx-auto p-6">
        <h2 className="text-3xl font-bold text-gray-800 dark:text-white mb-8 text-center">Our Products</h2>
        {products.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {products.map((product) => {
              const prominentImage = product.images?.find(img => img.isProminent)?.url || product.images?.[0]?.url;
              const isValidImage = prominentImage && prominentImage.startsWith('https://res.cloudinary.com');
              ...
              return (
                <div key={product._id} className="border rounded-lg shadow-lg overflow-hidden bg-white dark:bg-gray-800 hover:shadow-xl transition-shadow">
                  <Link href={`/products/${product._id}`} className="block">
                    ...
                    <div className="p-4">
                      <h3 className="text-lg font-semibold mb-1 text-gray-800 dark:text-white">{product.name}</h3>
      ...
      < Footer className="bottom foreground"/>
    </div>
  );
}